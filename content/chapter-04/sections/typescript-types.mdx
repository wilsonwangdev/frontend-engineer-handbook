...
>
> 整节读完约 60-80 分钟；按上面挑读 20-30 分钟。**§3 类型收窄**和**§5 泛型**是两个核心——它们决定了你写 TS 的天花板。
...
下一节：[§4.6 TypeScript 实战与类型设计](/chapter-04/typescript-design) _（待写）_——何时 `type` vs `interface`、何时不用 `enum`、如何让"非法状态无法表示"。
