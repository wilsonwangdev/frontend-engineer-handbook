...
通读约 50-60 分钟；按上述路径挑读 15-20 分钟。**§3 让非法状态无法表示** 和 **§4 类型边界** 是两个核心——它们决定了你的类型设计水平。
...
<Term k="brand-pattern">Brand pattern</Term>：用一个"假属性"区分类型——TS
编译期知道它们不同，但运行时它们就是 string。
