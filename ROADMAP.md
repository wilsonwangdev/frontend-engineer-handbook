...
- [x] ~~**第 5 章 React 与 Next.js**~~（2026-06-07 完整上线，六节齐全）
- [ ] **第 6 章 工程化与构建**
- [ ] **第 7 章 质量与交付**
...
- [x] ~~**附录 B** 精选一手资源 + 经典书目~~（2026-06-07：`/resources` 上线，含 12 个一手资源 + 6 本经典书目）
- [x] ~~**附录 C** 中英术语对照表~~（由 `/glossary` 承载——hover 浮窗 + 锚点索引 + 搜索可检索，比静态附录更强）
- [x] ~~**附录 D** 实战避坑录~~（2026-06-07：/appendix-d 上线，5 大类 12 条）
...
- [x] ~~**右侧 TOC**：章节页标题滚动跟踪~~（2026-06-07：IntersectionObserver + sticky sidebar）
- [x] ~~**阅读进度**：顶部进度条 + 章节完成度（localStorage）~~（2026-06-07）
- [x] ~~**面包屑**：章 > 节位置导航~~（2026-06-07：服务器端组件，含分隔线 + 移动端换行）
- [ ] **移动端**：抽屉式侧边栏 + 底部导航
...
- [x] ~~**PWA**：Service Worker + manifest + 离线缓存~~（2026-06-07：sw.js stale-while-revalidate + manifest.webmanifest + FE 图标）
- [ ] **导出**：PDF（Puppeteer 或 react-pdf）+ Markdown 打包（.zip）
- [x] ~~**首页打磨**：hero、路径卡片、特性栅格~~（2026-06-07）
- [x] ~~**分享功能**：标题锚点复制 + Web Share API~~（2026-06-07）
...
- [ ] **引导式阅读路径**：替代已废弃的 sidebar 路径切换方案。每条路径是一组
      有序节序列（像播放列表），而非对完整目录树的过滤。实现前提：
  - 路径数据（每条路径包含哪些节、按什么顺序）——当前不存在，写作时需逐节定义
  - 路径模式侧边栏：替换完整 ToC，只显示当前路径序列 + 进度指示
  - 节间导航按路径顺序走，而非章顺序
  - 路径选择入口可在首页或章节首页
...
- [ ] **快照测试 helper**：`e2e/utils/snapshot.ts`——为内容类页面统一断言
      风格，做一次复用所有章节
- [x] ~~**内容 frontmatter 检查**：CI 步骤，合并前对所有 `content/**/*.mdx`
      跑 `frontmatterSchema`。~~（2026-06-07：check-frontmatter.mjs 接入 CI）
...
- [x] ~~**Sitemap + robots.txt**：最基本的 SEO~~（2026-06-07：Next.js MetadataRoute）
...
| 债务                                                                                                                      | 创建于         | 关联 SPEC                                                          | 偿还计划  |
| ------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------------------------------------------------------------ | --------- |
| ~~`e2e/*.spec.ts` 用了具体文字断言~~（2026-06-07 已修复：具体断言改为 `.first()`、修复 a11y 颜色对比度、CI 恢复自动运行） | ~~2026-05-19~~ | [SPEC-0005](specs/0005-companion-tracks-and-test-strategy/spec.md) | ✅ 已偿还 |
