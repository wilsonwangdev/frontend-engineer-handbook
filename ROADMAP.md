...
- [ ] **顶级导航与公共页面入口**：按
      [SPEC-0015](specs/0015-top-level-navigation/spec.md)
      实施——header 加"词条" / "Playground"等公共页面入口，
      移动端抽屉同步分组。已有 `/glossary` 上线但缺导航入口，
      下次推进时优先做这个
- [ ] **整站 Playground 集中页系统**：按
      [SPEC-0016](specs/0016-playground-system/spec.md) 分两阶段：
  - 阶段 1：建 registry SSOT + `/playground` 索引 + `/playground/animations`
    （集成现有 4 个动画 demo）+ `/playground/layout`（3 个布局 demo）
  - 阶段 2：补 7 个动画 demo（Loading spinner / 骨架屏 shimmer / 手风琴
    `@starting-style` / transform vs left 性能对比 / View Transition
    命名捕获 / 滚动驱动进度条 / reduced-motion 对比）+ 难度筛选 + 章节回链
- [ ] **术语表维护方案演化**：按
