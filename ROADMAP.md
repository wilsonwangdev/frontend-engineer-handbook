...
**第 4 章 JavaScript 与 TypeScript**——见下方"下一步"。
...
- [ ] **第 4 章 JavaScript 与 TypeScript**
  - 进度：章首页 + §4.1 JavaScript 语言核心 + §4.2 异步模型 + §4.3
    模块系统 + §4.4 ES2020-2025 现代特性 已上；§4.5 TypeScript 类型
    系统 / §4.6 TypeScript 实战与类型设计 待写
...
- [ ] **动画 Playground 全集**：补齐 `/playground/animations` 页面 +
      剩余 7 个 demo（Loading spinner、骨架屏 shimmer、手风琴
      `@starting-style`、transform vs left 性能对比、View Transition
      命名捕获、滚动驱动进度条、reduced-motion 对比）。§3.6 章节内已
      嵌入 4 个核心 demo（按钮 hover / 卡片进入 / View Transition 基础 /
      暗色圆形扩散）；playground 是面向"想集中动手玩"的读者的扩展空间，
      未来其他章节的 demo 也进 `/playground/<topic>`
